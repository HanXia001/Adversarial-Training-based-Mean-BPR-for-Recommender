...
class MBPR:
    ...
    def main(self):
        ...
        for u in range(self.user_count): # 943
            for item in range(self.item_count): # 1682
                tbar_1.update(1)
        ...
        for t in range(max_re_count):
            ...
            np.savetxt("self.U_MBPR.txt",self.U)
            np.savetxt("self.V_MBPR.txt",self.V)
