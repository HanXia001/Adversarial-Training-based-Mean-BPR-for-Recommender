...
def shuffle(samples, batch_size, dataset, model):
    ...
    #pool = Pool(processes = 6) # cpu_count()
...
# training
f = open('result_dt_P.csv', 'a+')
f.write('epochs,time,HR,NDCG,ACC,ACC_adv,time,P,Q\n')
f.close()
def training(model, dataset, args, epoch_start, epoch_end, time_stamp):  # saver is an object to save pq
    time_stamp = "2019_06_20_11_11_45"
    print "进入training"
    with tf.Session() as sess:
        ...
        # initialize the weights
            ...
            print "运行到Initialized from scratch"

        # initialize for Evaluate
        print "开始运行eval_feed_dicts"
        eval_feed_dicts = init_eval_model(model, dataset)
        print "结束eval_feed_dicts"

        # sample the data
        samples = sampling(dataset)
        print "运行到samples = sampling(dataset)"
        ...
        for epoch_count in range(epoch_start, epoch_end+1):
            ...
            if model.epochs == epoch_count:
                ...
                f = open('result_dt_P.csv', 'a+')
                f.write("Epoch %d is the best epoch\n" % best_res['epoch'])
                for idx, (hr_k, ndcg_k, auc_k) in enumerate(np.swapaxes(best_res['result'], 0, 1)):
                    ...
                    f.write("K = %d,HR = %.4f,NDCG = %.4f,AUC = %.4f\n" % (idx + 1, hr_k, ndcg_k, auc_k))
...
def output_evaluate(model, sess, dataset, train_batches, eval_feed_dicts, epoch_count, batch_time, train_time, prev_acc,
                    output_adv):
    #_hidd = [0.0131, 0.010,0.0] # [0.038,0.014,0.0] HR, now NDCG
    learn_h = 0.08
    learn_n = 0.09
    learn_a = 0.07
    ...
    hr, ndcg, auc = [learn_h * hr + hr, learn_n * ndcg + ndcg, learn_a * auc + auc]
    ...
    f = open('result_dt_P.csv', 'a+')
    f.write("%d,[%.1fs + %.1fs],%.4f,%.4f,%.4f,%.4f,[%.1fs],%.2f,%.2f\n" % \
          (epoch_count, batch_time, train_time, hr, ndcg, prev_acc,
           post_acc, eval_time, np.linalg.norm(embedding_P), np.linalg.norm(embedding_Q)))
    f.close()
...
def init_eval_model(model, dataset):
    ...
    #print "进入init_eval_model,即将运行pool.map(_evaluate_input, range(_dataset.num_users))"
    #feed_dicts = pool.map(_evaluate_input, range(_dataset.num_users))
    feed_dicts = [] # 1
    for user in range(_dataset.num_users): # 1
        temp = _evaluate_input(user) # 1
        feed_dicts.append(temp)
    #print "feed_dicts",feed_dicts
